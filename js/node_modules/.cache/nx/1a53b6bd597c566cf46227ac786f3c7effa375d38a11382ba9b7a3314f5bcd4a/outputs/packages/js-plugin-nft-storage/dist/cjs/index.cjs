...
from({
  name: 'sha2-512',
  code: 19,
  encode: input => coerce(crypto__default["default"].createHash('sha512').update(input).digest())
});
...
class NftStorageDriver {
  ...
  async client() {
    ...
    return js.isKeypairSigner(signer) ? metaplexAuth.NFTStorageMetaplexor.withSecretKey(signer.secretKey, authOptions) : metaplexAuth.NFTStorageMetaplexor.withSigner(signer.signMessage.bind(signer), signer.publicKey.toBuffer(), authOptions);
  }
